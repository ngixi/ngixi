...
function configureDawnCMake(cwd, buildDir, installPrefix) {
  ...
  const cmakeArgs = [
    ...
    // Force MSVC compiler (prevents GCC/Clang flags from being used)
...
function buildDawnCMake(cwd, buildDir) {
  ...
  const buildArgs = [
    "--build", path.relative(cwd, buildDir),
    "--config", "Release",
    "--parallel",
  ];
...
function installDawnArtifacts(cwd, buildDir) {
  ...
  const installArgs = [
    "--install", path.relative(cwd, buildDir),
    "--config", "Release",
  ];
...
function verifyDawnArtifacts(artifactsRoot) {
  ...
  const keyHeaders = [
    ...
    path.join(includeDir, "dawn", "dawn_proc.h"),
  ];
  ...
  if (process.platform === "win32") {
    const keyLibs = [
      ...
      path.join(libDir, "dawn_proc.lib"),
    ];
